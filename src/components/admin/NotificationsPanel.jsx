...
import { Bell, Send, Trash2, Search, Loader2, Clock, AlertCircle } from 'lucide-react';
...
const NotificationsPanel = () => {
  ...
  return (
    <div className="space-y-6 pb-28 sm:pb-36 max-w-full overflow-hidden">
      {/* Top Header */}
      ...
      <div className="grid gap-6 grid-cols-1 lg:grid-cols-[minmax(0,1.2fr)_minmax(380px,0.8fr)] items-start">
        ...
        <div className="space-y-4 min-w-0">
          <div className="relative">
            ...
            <input
              ...
              className="h-11 w-full rounded-2xl border border-slate-200 bg-white pl-10 pr-4 text-xs font-semibold focus:outline-none focus:border-primary shadow-2xs transition-colors"
            />
          </div>

          <div className="space-y-3 min-w-0">
            {paginated.data.length === 0 ? (
              <div className="rounded-3xl border border-slate-200/80 bg-white p-8 sm:p-12 text-center space-y-3 shadow-2xs">
                <div className="h-14 w-14 mx-auto rounded-2xl bg-blue-50 text-primary flex items-center justify-center text-2xl font-black shadow-inner">
                  📢
                </div>
              ...
              paginated.data.map((notif) => {
                const formattedTime = formatDate(notif.created_at);
                return (
                  <div
                    key={notif.id}
                    className="rounded-2xl border border-slate-200/90 bg-white p-4 shadow-2xs space-y-3 hover:border-slate-300 transition-all min-w-0"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-2.5 min-w-0">
                        <div className="h-9 w-9 rounded-xl bg-blue-50 text-primary flex items-center justify-center shrink-0 shadow-2xs">
                          <Bell size={16} />
                        </div>
                        <div className="min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1 flex-wrap">
                            <span
                              className={`rounded-full px-2.5 py-0.5 text-[10px] font-black uppercase tracking-wider border ${getTargetBadgeClass(
                                notif.target_role
                              )}`}
                            >
                              Audience: {getTargetRoleLabel(notif.target_role)}
                            </span>
                            {formattedTime && (
                    ...
                    <p className="text-xs text-slate-700 bg-slate-50/80 p-3 rounded-xl border border-slate-100 font-medium leading-relaxed break-words">
                      {notif.message}
                    </p>
                  </div>
                );
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-700 disabled:opacity-40 cursor-pointer hover:bg-slate-50 transition-colors"
                >
                  Previous
                </button>
                <button
        ...
        >
          ...
          <div>
            <label className="text-xs font-extrabold text-slate-700 block mb-1.5">TARGET AUDIENCE</label>
            <select
              ...
              className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold text-slate-800 focus:outline-none focus:border-primary cursor-pointer transition-colors w-full min-w-0 overflow-hidden text-ellipsis"
            >
          ...
          {/* Notification Title */}
          <div>
            <label className="text-xs font-extrabold text-slate-700 block mb-1.5">Notification Title</label>
            <input
              type="text"
              value={form.title}
              onChange={(e) => {
              ...
              className={`h-11 w-full rounded-xl border px-3 text-xs font-semibold focus:outline-none transition-colors ${
                errors.title ? 'border-rose-400 bg-rose-50/60 text-rose-900' : 'border-slate-200 focus:border-primary'
              }`}
            />
            {errors.title && (
          ...
          <div>
            ...
            <textarea
              ...
              rows={5}
              className={`w-full min-h-[130px] rounded-xl border p-3 text-xs font-semibold focus:outline-none transition-colors resize-y ${
                errors.message ? 'border-rose-400 bg-rose-50/60 text-rose-900' : 'border-slate-200 focus:border-primary'
              }`}
            />
            {errors.message && (
              <p className="text-[11px] font-bold text-rose-600 mt-1.5 flex items-center gap-1">
          ...
          <button
            type="submit"
            disabled={isSending}
            className="w-full py-3.5 px-4 rounded-xl bg-primary text-white text-xs font-black uppercase tracking-wider shadow-md hover:bg-blue-700 active:scale-[0.99] transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed disabled:transform-none mt-2"
          >
            {isSending ? (
              <>
